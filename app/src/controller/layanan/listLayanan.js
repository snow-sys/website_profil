/** mengatur request untuk men-list layanan */

let { listLayanan } = require("../../repository/layanan");
...
module.exports = (req, res) => {
  ...
  console.log(`[${formatted_date}] : Start listLayanan query...`);
  listLayanan(uid, limit, from, cari)
    .then((data) => {
      console.log(
        `[${formatted_date}] : finish listLayanan query! Start mapping data...`
      );
      ...
      res.json({
        message: "berhasil menampilkan list layanan",
        code: res.statusCode,
        response: data,
      });
    })
    .catch((err) => {
      console.log(`[${formatted_date}] : listLayanan query failed!`);
      console.log(err);
      res.json({
        message: "gagal menampilkan list layanan",
        code: res.statusCode,
        response: err,
      });
    });
